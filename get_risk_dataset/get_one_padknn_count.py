    ...
    for k in k_list:
        ...
        for data_set in data_sets:
            ...
            distribution_train = pd.read_csv(
                join(csv_dir, "padding_{}_{}.csv".format(layer, data_sets[0])),
                header=None,
            ).to_numpy()
            ...
            distribution = pd.read_csv(
                join(csv_dir, "padding_{}_{}.csv".format(layer, data_set)),
                header=None,
            ).to_numpy()
            ...
            for i in range(len(p_d)):
                for j in range(k):
                    knn[i, j] = labels_train[np.where(p_d[i] == s_p_d[i, j+1])[0][0]]
        ...
        header = ["data", "label", "{}_{}_padcount{}".format(elem_name, layer, k)]

        # Save the final csv
        count_all.insert(0, header)
        pd.DataFrame(count_all).to_csv(
            os.path.join(csv_dir, "{}_{}_one_padknn{}.csv".format(elem_name, layer, k)),
            header=None, index=None
        )

        print("--- padknn_{} {:.2f} s ---".format(k, time() - start))
...
if __name__ == "__main__":
    ...
    layers = ['x4', "xc"]
    elem_name_str = "{}"
    csv_dir_str = '/home/ssd0/lfy/result_archive/chest_xray_e128_r50/'
