...
def get_distance(
    ...
    coordinate_train = pd.read_csv(
        os.path.join(csv_dir, "padding_{}_{}.csv".format(layer, data_sets[0])),
        header=None,
    ).to_numpy()
    label = pd.read_csv(
    ...
    for data_set in data_sets:
        ...
        coordinates = pd.read_csv(
            os.path.join(csv_dir, "padding_{}_{}.csv".format(layer, data_set)),
            header=None,
        ).to_numpy()
    ...
    for i in range(num_class):
        header.append("{}_{}_class_{:0>3d}_paddingdis".format(elem_name, layer, i))

    # Save the final csv
    distance_center_all.insert(0, header)
    pd.DataFrame(distance_center_all).to_csv(
        os.path.join(csv_dir, "{}_{}_paddingdis.csv".format(elem_name, layer)),
        header=None,
        index=None,
    )
...
if __name__ == "__main__":
    ...
    for layer in layers:
        ...
        for cnn in cnns:
            ...
            get_distance(layer, elem_name, num_class, csv_dir, "cosine")
