...
def get_one_distance(
    ...
    header = ["data", "label", "{}_{}_paddingdis".format(elem_name, layer)]

    # Save the final csv
    distance_center_all.insert(0, header)
    pd.DataFrame(distance_center_all).to_csv(
        os.path.join(csv_dir, "{}_{}_one_paddingdis.csv".format(elem_name, layer)),
...
if __name__ == "__main__":
    ...
    for layer in layers:
            ...
            for cnn in cnns:
                ...
                get_one_distance(
                    layer, elem_name, num_class, csv_dir, "cosine", data_sets
                )
