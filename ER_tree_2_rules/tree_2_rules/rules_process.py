...
class Rule:
            ...
            for condition in origin_conditions:
                ...
                if "<=" in condition_des[0]:
                    ...
                    logging.raiseExceptions(
                        'Unknown comparator! Only "<=" or ">" allowed.'
                    )
                # Attribute metric
                ...
                readable_conditions.append(
                    elems[0] + compare_op + str(round(float(elems[1]), 4))
                )
    ...
    def __gt__(self, other):
        ...
        if condition_str1 in condition_str2:
            return True
        # complicate condition comparison: if conditions of r_1 dominate conditions of r_2, then r_1 > r_2.
    ...
    def apply(self, attrs_2_values):
        ...
        :return:
        """
        if attrs_2_values is None or len(attrs_2_values.keys()) != len(
            self.involved_attributes
        ):
            logging.warning(
                "The number of input attributes are not equal to {}'s requirement!".format(
                    self.id
                )
            )
            return 0
        for condition in self.conditions.values():
            ...
            if attr_value is None:
                logging.warning(
                    "The value of attribute {} is None!".format(condition[0])
                )
                return 0
...
def clean_rules(rules, print_info=False):
    ...
    for i in range(len(rules)):
        ...
        if print_info and i >= _interval and (i % _interval == 0 or i == rule_size):
            print(
                "Progress of remove same rules: {:.0%}, {}s".format(
                    i / rule_size, time.time() - _start_time
                )
            )
            _start_time = time.time()
...
def merge_rules(rules, print_info=False):
    ...
    for i in range(rule_size - 1):
        ...
        for j in range(i + 1, rule_size):
            ...
            ):  # for rule comparison, the shorter rule is larger than the longer one.
        ...
        if print_info and i >= _interval and (i % _interval == 0 or i == rule_size):
            print(
                "Progress of de-duplicate rules: {:.0%}, {}s".format(
                    i / rule_size, time.time() - _start_time
                )
            )
            _start_time = time.time()
...
def clean_rules_mt(rules, process_number=1):
    """
    ...
    return_rules = pool.map(
        clean_rules, [rules[(i * bs) : min((i + 1) * bs, n)] for i in range(batch_num)]
    )
    pool.close()
    pool.join()
...
def test_rule_apply():
    ...
    k = 0
    print(
        "{} with {}: {}".format(rules[k].id, test_data[0], rules[k].apply(test_data[0]))
    )
    print(
        "{} with {}: {}".format(rules[k].id, test_data[1], rules[k].apply(test_data[1]))
    )
    k = 1
    print(
        "{} with {}: {}".format(rules[k].id, test_data[2], rules[k].apply(test_data[2]))
    )
    k = 2
    print(
        "{} with {}: {}".format(rules[k].id, test_data[3], rules[k].apply(test_data[3]))
    )
...
def test_dominate_clean():
    ...
    rules = [
        ...
        "attr2<=0.3:M|0|0|0",
        "attr2<=0.5:M|0|0|0",
        "attr3>0.9:M|0|0|0 && attr1>0.8:M|0|0|0",
        "attr4<=0.2:M|0|0|0 && attr3>0.9:M|0|0|0 && attr1>0.8:U|0|0|0",
        "attr5>0.6:M|0|0|0 && attr6>0.6:M|0|0|0",
        "attr6>0.4:M|0|0|0",
    ]
    save_rules(f_path, rules)
    rules_ = read_rules(f_path)
