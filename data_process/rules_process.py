...
def read_rules(file_path):
    ...
    for line in file_ob.readlines():
        ...
        if rule_description in existing_items:
            continue
        #if 'count' in rule_description:
        #    continue
        if '50' not in rule_description:
            continue
        rule_id = 'rule_' + str(i)
...
def merge_rules(rules, print_info=False):
    ...
    for i in range(rule_size-1):
        ...
        for j in range(i+1, rule_size):
            ...
            if probe_rule > target_rule:  # for rule comparison, the shorter rule is larger than the longer one.
...
def clean_rules_mt(rules, process_number=5):
    ...
    results = clean_rules(rules_temp, False)
    return results
